...
import com.sjjd.wyl.ebooks.utils.TextureUtils;
...
public class FlipCards {
    ...
    private static final int MAX_TIP_ANGLE = 0;//第一页 和最后一页 能翻起来的角度  默认为0，禁止翻动
    ...
    public void reloadTexture(int frontIndex, View frontView, int backIndex, View backView) {
        synchronized (this) {
            ...
            if (AphidLog.ENABLE_DEBUG) {
                AphidLog
                        .e("reloading texture: %s and %s; old views: %s, %s, front changed %s, back changed %s",
                                frontView, backView, frontCards.getView(), backCards.getView(), frontChanged,
                                backChanged);
            }

            if (AphidLog.ENABLE_DEBUG) {
                AphidLog.e("reloadTexture: activeIndex %d, front %d, back %d, angle %.1f",
